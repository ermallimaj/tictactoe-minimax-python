...
class Game:
    ...
    def __init__(self, player='X', computer='O', difficulty='impossible'):
        self.player = player
    ...
    def calculate_move(self):
        if self.difficulty == 'impossible':
            return self.calculate_impossible_move()
        elif self.difficulty == 'easy':
            return self.random_available_move()

        else:
            raise ValueError("Invalid difficulty. Choose from 'impossible' or 'easy'.")

    def calculate_impossible_move(self):
        ...
        if not move[1]:
            return {'row': -1, 'col': -1}  # Indicate invalid move with negative indices
        else:
            return {'row': move[1][0], 'col': move[1][1]}

    def random_available_move(self):
        moves = self.get_moves()
        if moves:
            return {'row': moves[0][0], 'col': moves[0][1]}
        else:
            return {'row': -1, 'col': -1}  # Indicate invalid move with negative indices
